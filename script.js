const TicTac = {
    cPlayer: "X", // Tracks current player (X or O)
    state: Array(9).fill(null), // Board state (null for empty cells)
    gameOver: false, // Indicates if the game is over

    // Initialize the game
    init() {
        this.cBoard();
        document
            .getElementById("reset")
            .addEventListener("click", () => this.reset());
    },

    // Create the game board dynamically
    cBoard() {
        const board = document.getElementById("board");
        board.innerHTML = ""; // Clear previous board
        this.state.forEach((_, i) => {
            const cell = document.createElement("div");
            cell.classList.add("cell");
            cell.dataset.index = i;
            board.appendChild(cell);
        });
        board.addEventListener("click", (e) => this.handleClick(e)); // Handle clicks on the board
        this.uMessage(`Player ${this.cPlayer}'s turn`);
    },

    // Handle a cell click
    handleClick(e) {
        const cell = e.target;
        const i = cell.dataset.index;

        // Ignore clicks if game is over or cell is taken
        if (this.gameOver || !cell.classList.contains("cell") || this.state[i])
            return;

        // Update board state and UI
        this.state[i] = this.cPlayer;
        cell.textContent = this.cPlayer;
        cell.classList.add("taken");

        // Check for winner or tie
        const winCombo = this.checkWin();
        if (winCombo) {
            this.highlight(winCombo);
            this.uMessage(`AI Wins 😘✨`);
            this.gameOver = true;
        } else if (this.state.every((cell) => cell)) {
            this.uMessage("It's a tie! 😊😊");
            this.gameOver = true;
        } else {
            // Switch players
            this.cPlayer = this.cPlayer === "X" ? "O" : "X";
            this.uMessage(`Player ${this.cPlayer}'s turn`);
            if (this.cPlayer === "O") {
                const [row, col] = this.computerMove();
                this.handleClick({ target: document.querySelector(`[data-index="${row * 3 + col}"]`) });
            }
        }
    },

    // Check if there's a winning combination
    checkWin() {
        const wins = [
            [0, 1, 2],
            [3, 4, 5],
            [6, 7, 8], // Rows
            [0, 3, 6],
            [1, 4, 7],
            [2, 5, 8], // Columns
            [0, 4, 8],
            [2, 4, 6], // Diagonals
        ];
        return wins.find((combo) =>
            combo.every((i) => this.state[i] === this.cPlayer)
        );
    },

    // Highlight winning cells
    highlight(combo) {
        combo.forEach((i) => {
            document.getElementById("board").children[i].style.background = "red";
        });
    },

    // Reset the game
    reset() {
        this.state = Array(9).fill(null);
        this.cPlayer = "X";
        this.gameOver = false;
        this.cBoard();
    },

    // Update the game status message
    uMessage(msg) {
        document.getElementById("message").textContent = msg;
    },

    // Evaluation function to check the board's utility
    evaluate() {
        // Checking for Rows
        for (let row = 0; row < 3; row++) {
            if (this.state[row * 3] === this.state[row * 3 + 1] && this.state[row * 3] === this.state[row * 3 + 2]) {
                if (this.state[row * 3] === "X") return -1;
                if (this.state[row * 3] === "O") return 1;
            }
        }

        // Checking for Columns
        for (let col = 0; col < 3; col++) {
            if (this.state[col] === this.state[col + 3] && this.state[col] === this.state[col + 6]) {
                if (this.state[col] === "X") return -1;
                if (this.state[col] === "O") return 1;
            }
        }

        // Checking Diagonals
        if (this.state[0] === this.state[4] && this.state[0] === this.state[8]) {
            if (this.state[0] === "X") return -1;
            if (this.state[0] === "O") return 1;
        }
        if (this.state[2] === this.state[4] && this.state[2] === this.state[6]) {
            if (this.state[2] === "X") return -1;
            if (this.state[2] === "O") return 1;
        }

        // No winner
        return 0;
    },

    // Check if the move is valid (empty space)
    canMove(i) {
        return this.state[i] === null;
    },

    // Get all available positions for the next move
    getAllAvailablePositions() {
        const availablePositions = [];
        for (let i = 0; i < 9; i++) {
            if (this.state[i] === null) {
                availablePositions.push(i);
            }
        }
        return availablePositions;
    },

    // Minimax with Alpha-Beta Pruning
    minimax(alpha, beta, isMaximizingPlayer) {
        const score = this.evaluate();

        // Maximizing for "O" (computer)
        if (score === 1) return { score: score };
        // Minimizing for "X" (player)
        if (score === -1) return { score: score };
        // Check for draw
        if (this.getAllAvailablePositions().length === 0) return { score: 0 };

        let bestMove = null;

        if (isMaximizingPlayer) {
            let bestScore = -Infinity;
            const availablePositions = this.getAllAvailablePositions();
            for (const pos of availablePositions) {
                this.state[pos] = "O";
                const { score } = this.minimax(alpha, beta, false);
                this.state[pos] = null;
                if (score > bestScore) {
                    bestScore = score;
                    bestMove = pos;
                }
                alpha = Math.max(alpha, score);
                if (beta <= alpha) break; // Beta cut-off
            }
            return { score: bestScore, move: bestMove };
        } else {
            let bestScore = Infinity;
            const availablePositions = this.getAllAvailablePositions();
            for (const pos of availablePositions) {
                this.state[pos] = "X";
                const { score } = this.minimax(alpha, beta, true);
                this.state[pos] = null;
                if (score < bestScore) {
                    bestScore = score;
                    bestMove = pos;
                }
                beta = Math.min(beta, score);
                if (beta <= alpha) break; // Alpha cut-off
            }
            return { score: bestScore, move: bestMove };
        }
    },

    // Get the computer's next move
    computerMove() {
        const { move } = this.minimax(-Infinity, Infinity, true);
        const row = Math.floor(move / 3);
        const col = move % 3;
        return [row, col];
    },
};

// Start the game
TicTac.init();
